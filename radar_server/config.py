...
class ForecastProduct:
    ...
    # The semi-Lagrangian warp integrates the (smooth) displacement trajectory on
    # a grid coarsened by this factor and upscales it for the full-resolution
    # precip warps; >1 roughly halves the warp cost for a tiny accuracy cost
    # (~0.08 dBZ at 2). 1 keeps the full-resolution trajectory. Only applies on
    # the fast_warp path.
    warp_grid_step: int = 2
